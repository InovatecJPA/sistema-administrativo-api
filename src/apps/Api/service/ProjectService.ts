...
export class ProjectService implements ServiceInterface<Project, ProjectDto> {
    ...
    async save(objectDto: Partial<ProjectDto>): Promise<Project> {
        if(!objectDto.isValid()) {
            throw new InvalidObjectError('All fields of the new projecyt must be non-null or different of "" .');
        }
...
export const projectService: ProjectService = new ProjectService(projectRepository);
