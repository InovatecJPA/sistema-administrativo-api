export default class CpfValidator {
  ...
  private static generateValidCpf(cpfDigitsString: string) {
    ...
    const firstValidatorDigit: string =
      CpfValidator.generateDigit(cpfWithoutDigits);
    const secondValidatorDigit: string = CpfValidator.generateDigit(
      cpfWithoutDigits + firstValidatorDigit
    );
